...
namespace WWFontEditor.Domain.FontTypes
{
    ...
    /// </summary>
    public class FontFileDynV1b : FontFile
    {
        ...
        public override String LongTypeDescription { get { return "An 8-pixel wide, 96-symbol, 1 BPP font, which is saved as 4bpp, but with 0 and F as only used value. Doesn't have any kind of file header."; } }
        ...
        {
            if (fileData.Length == 0 || fileData.Length % (4 * 0x60) != 0)
                throw new FileTypeLoadException(ERR_SIZECHECK);
            this.m_FontWidth = 8;
            this.m_FontHeight = fileData.Length / (4 * 0x60);
            Byte startSymbol = 32;
            ...
            Int32 symbolsize = m_FontHeight * 4;
            for (Int32 i = 0; i < nrOfSymbols; i++)
            {
                Byte[] curData8bit = ImageUtils.ConvertTo8Bit(fileData, m_FontWidth, m_FontHeight, symbolsize * i, 4, true);
                for (Int32 b = 0; b < curData8bit.Length; b++)
                {
                    ...
                    {
                        if (val != 0x0F)
                            throw new FileTypeLoadException("Dynamix v1b only accepts 0 and F as values");
                        else
                            curData8bit[b] = 1;
                    }
                }
        ...
        public override Byte[] SaveFont(Boolean disableCompression)
        {
            ...
            for (Int32 i = 0; i < 0x60; i++)
            {
                ...
                imageData[i] = ImageUtils.ConvertFrom8Bit(eightBitData, this.m_FontWidth, this.m_FontHeight, 4, true);
            }
